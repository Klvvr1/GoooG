...
export const CharacterCard: React.FC<CharacterCardProps> = ({
  ...
  return (
    <div
      className={`group relative flex flex-col justify-between rounded-2xl bg-stone-900/90 border transition-all duration-300 overflow-hidden ${
        character.enabled
          ? 'border-stone-800 hover:border-stone-700 shadow-md hover:shadow-xl'
          : 'border-stone-800/40 opacity-60 bg-stone-950/80'
      }`}
    >
      {/* Top Image & Overlay */}
      <div className="relative aspect-[4/5] w-full overflow-hidden bg-stone-950">
        ...
        <button
          ...
          className="absolute bottom-3 left-3 flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-black/70 backdrop-blur-md text-xs font-semibold text-stone-200 border border-white/10 hover:bg-black/90 transition-colors"
        >
            ...
            {images.map((_, idx) => (
              <button
                ...
                className={`w-1.5 h-1.5 rounded-full transition-all ${
                  idx === selectedPhotoIdx ? 'w-3.5 bg-amber-400' : 'bg-white/40'
                }`}
              />
            ))}
          </div>
      ...
      {showPhotoViewer && images.length > 1 && (
        <div className="p-2.5 bg-stone-950/90 border-t border-b border-stone-800 flex items-center gap-2 overflow-x-auto">
          {images.map((img, idx) => (
            <img
              key={idx}
              src={img}
              ...
              className={`w-12 h-14 object-cover rounded-lg cursor-pointer flex-shrink-0 border-2 transition-all ${
                idx === selectedPhotoIdx ? 'border-amber-500 scale-105' : 'border-transparent opacity-70 hover:opacity-100'
              }`}
            />
          ))}
        </div>
      ...
      <div className="p-4 space-y-3">
          ...
          <div className="flex flex-col items-end">
            <button
              ...
              className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
                character.enabled ? 'bg-amber-600' : 'bg-stone-700'
              }`}
        ...
        <div className="flex items-center gap-2 pt-2 border-t border-stone-800/80">
          <button
            ...
            className="flex-1 py-1.5 px-3 rounded-lg bg-stone-800 hover:bg-stone-700 text-stone-200 text-xs font-semibold flex items-center justify-center gap-1.5 transition-colors"
          >
