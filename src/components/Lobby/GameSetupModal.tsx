...
export const GameSetupModal: React.FC<GameSetupModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="relative w-full max-w-lg rounded-3xl bg-stone-900 border border-stone-800 shadow-2xl p-6 sm:p-8 space-y-6 overflow-hidden">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            >
              <div className="flex items-center justify-between">
                <Target className={`w-5 h-5 ${selectedMode === 'classic' ? 'text-amber-400' : 'text-stone-400'}`} />
                {selectedMode === 'classic' && (
            ...
            >
              <div className="flex items-center justify-between">
                <Split className={`w-5 h-5 ${selectedMode === 'match' ? 'text-amber-400' : 'text-stone-400'}`} />
                {selectedMode === 'match' && (
                  <span className="w-2 h-2 rounded-full bg-amber-400 animate-ping" />
                )}
              </div>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-stone-400">
              Available pool: <strong className="text-stone-200">{maxAvailable} characters</strong>
            </span>
          </div>
        ...
        <div className="pt-2">
          <button
            ...
            className="w-full py-3.5 px-6 rounded-2xl bg-gradient-to-r from-amber-600 to-orange-600 hover:from-amber-500 hover:to-orange-500 disabled:opacity-50 disabled:cursor-not-allowed text-white font-bold text-base shadow-xl shadow-amber-600/30 hover:shadow-amber-500/50 flex items-center justify-center gap-2 transition-all"
          >
