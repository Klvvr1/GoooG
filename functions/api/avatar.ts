...
export async function onRequestGet(context: { request: Request }): Promise<Response> {
  ...
  // Only allow proxying from cdni.pornpics.com/models/ path
  if (!imageUrl.startsWith('https://cdni.pornpics.com/models/')) {
    return new Response('Forbidden: only models/ path allowed', { status: 403, headers: CORS_HEADERS });
  }
